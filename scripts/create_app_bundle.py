#!/usr/bin/env python3
"""
Create macOS Application Bundle for Music Visualizer
...
def create_app_bundle():
	...
	# - Include the source package so we can import `music_visualizer`
	root = Path.cwd()
	src_dir = root / "src"
	if src_dir.exists():
		# Copy the entire src tree (contains `music_visualizer` package)
	...
	# No database required for Music Visualizer
...
# Ensure we can import the `music_visualizer` package from Resources/src
...
try:
    ...
    from music_visualizer import main as run_main
    run_main()
...
"""
	...
	plist_data = {
		...
		'CFBundleIdentifier': 'com.music_visualizer.app',
		'CFBundleVersion': '1.0.0',
		'CFBundleShortVersionString': '1.0.0',
		'CFBundleExecutable': app_name.replace(" ", ""),
		'CFBundleIconFile': 'music_visualizer.icns',
...
def create_icns_icon(resources_path):
	...
	try:
		...
		iconset_dir = resources_path / "music_visualizer.iconset"
		iconset_dir.mkdir(exist_ok=True)

		# Icon size mappings for macOS
		icon_mappings = {
			'icon_16x16.png': 'music_visualizer_16x16.png',
			'icon_16x16@2x.png': 'music_visualizer_32x32.png',
			'icon_32x32.png': 'music_visualizer_32x32.png',
			'icon_32x32@2x.png': 'music_visualizer_64x64.png',
			'icon_128x128.png': 'music_visualizer_128x128.png',
			'icon_128x128@2x.png': 'music_visualizer_256x256.png',
			'icon_256x256.png': 'music_visualizer_256x256.png',
			'icon_256x256@2x.png': 'music_visualizer_512x512.png',
			'icon_512x512.png': 'music_visualizer_512x512.png',
			'icon_512x512@2x.png': 'music_visualizer_1024x1024.png'
		}
		...
		icns_path = resources_path / "music_visualizer.icns"
		...
		if result.returncode == 0:
			...
			largest_icon = icons_dir / "music_visualizer_512x512.png"
			if largest_icon.exists():
				shutil.copy2(largest_icon, resources_path / "music_visualizer.icns")

	except Exception as e:
		print(f"⚠️  Could not create ICNS icon: {e}")
		# Copy any available icon as fallback
		if (icons_dir / "music_visualizer.png").exists():
			shutil.copy2(icons_dir / "music_visualizer.png", resources_path / "music_visualizer.icns")
